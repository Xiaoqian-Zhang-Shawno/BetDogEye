...
- 模型 API 分析：通过本地 Python 代理调用 OpenAI-compatible Chat Completions API，支持 DeepSeek、GPT/OpenAI 和其他兼容服务。
- 网络情报刷新：查询公开新闻 RSS，将来源标题、摘要、发布时间和链接交给模型归纳为可继续分析的情报文本。
- 未结算组合刷新：只针对已下注且未结算记录中的球队/国家搜索最新信息，覆盖伤停、政治、金融、花边舆情、更衣室和市场异常因素。
...
项目不会把 API Key 写入前端代码。浏览器只请求本机 `/api/llm/*` 和 `/api/intel/*`，由 `server.py` 在本机读取密钥后转发到 OpenAI-compatible 模型接口。

### 方法一：在页面中配置

1. 启动本地服务。
2. 打开「风险情报」页。
3. 在模型 API 区域选择供应商并输入 API Key。
4. DeepSeek 默认 Base URL：
...
如果使用 OpenAI/GPT，Base URL 通常为：

```text
https://api.openai.com/v1
```

模型名请填写你的账号可用模型，例如 GPT 系列模型名。
...
点击「刷新网络情报」会针对当前选择的比赛执行：

1. 根据当前比赛、投注方向和输入框里的补充关键词搜索公开新闻 RSS。
2. 抽取新闻标题、摘要、发布时间和链接。
3. 调用模型将搜索结果归纳为一段赛前情报文本。
4. 自动填入情报文本框，并在右侧展示引用来源。

点击「刷新未结算组合」会执行：

1. 读取所有状态为「未结算」的下注记录。
2. 从比赛名、投注方向、相关性分组中提取球队/国家关键词。
3. 分别搜索伤停、阵容、政治/地缘、足协/签证、金融/赞助、工资奖金、花边舆情、更衣室纪律和赔率市场异常。
4. 服务端先按球队/国家关键词过滤来源，减少无关新闻。
5. 模型再次按“只保留与未结算下注相关信息”的规则归纳。
6. 自动生成组合级情报文本，并展示引用来源。

刷新后可以继续点击「挖掘风险因子」或「模型分析」。

点击「模型分析」会通过本地代理调用已配置模型，返回：
...
- 前端代码不包含真实 API Key。
- 模型分析只发送当前输入的情报文本、已选比赛、投注方向和组合摘要。
- 单场网络情报刷新会向公开新闻/RSS 服务发送比赛名、投注方向和补充关键词。
- 未结算组合刷新会向公开新闻/RSS 服务发送未结算下注记录中的球队/国家关键词。
- 网络情报刷新会把新闻搜索结果的标题、摘要、链接和发布时间发送给已配置模型归纳。
- 不会上传完整 JSON 备份文件。
